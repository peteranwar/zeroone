...
const TransferItemList = ({ itemData, status, hideActionColumn = false }) => {
  ...
  const TABLE_HEAD = [
    ...
    ...(!hideActionColumn ? [{ id: '', label: t('shard.action'), width: 88 }] : []),
  ];
  ...
  const handleDeleteRow = async id => {
    if (!id) return;
    try {
      await TransferOrderApiEndpoints.deleteItem({ transfer_id, item_id: id });
      ToastSuccess(t('inventory.purchase.details.items.deleteToast'));

      queryClient.invalidateQueries({ queryKey: ['transfer-id'] });
    } catch (error) {
      toast.error(error || t('validation.toastError'));
    }
  };
  ...
  return (
    ...
    >
              ...
              {itemData &&
                ...
                itemData?.map(row => (
                  <ItemTableRow
                    ...
                    onEditRow={() =>
                      setOpen(prev => ({ ...prev, open: true, id: row?.item?.id, row }))
                    }
                  />
                ))}
      ...
      >
        <ItemForm id={open?.id} setOpen={setOpen} rowData={open?.row} />
      </MainModal>
    </Box>
  );
};
