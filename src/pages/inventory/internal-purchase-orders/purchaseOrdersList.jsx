...
const PurchaseOrdersList = () => {
  ...
  const TABLE_HEAD = [
    ...
    { id: 'source', label: t('inventory.purchase.table.warehouse'), width: 120 },
    { id: 'destination', label: t('inventory.purchase.table.branch'), width: 120 },
  ...
  const internalPurchasesData = {
    ...purchasesData,
    data: purchasesData?.data
      ? purchasesData.data.filter(item => item?.type === 'internal')
      : [],
  };
      ...
      <Container maxWidth={settings.themeStretch ? false : 'xl'}>
        ...
        >
          <Stack alignItems='end'>
            ...
            >
              {t('inventory.purchase.addInternal')}
            </Button>
          </Stack>
            ...
            <Scrollbar>
              <Table sx={{ minWidth: 960 }}>
                <TableHeadCustom
                  ...
                  rowCount={internalPurchasesData?.data?.length}
                />

                <TableBody>
                  {isLoading && [1, 2, 3, 4].map(el => <TableSkeleton key={el} />)}
                  {internalPurchasesData?.data &&
                    internalPurchasesData?.data?.length > 0 &&
                    internalPurchasesData?.data?.map(row => (
                      <PurchaseTableRow
                  ...
                  <TableNoData
                    ...
                    notFound={internalPurchasesData?.data?.length < 1}
                  />
                </TableBody>
              </Table>
            </Scrollbar>
          ...
          <TablePaginationCustom
            count={
              internalPurchasesData?.pagination?.meta?.page?.total ||
              internalPurchasesData?.data?.length
            }
            page={+page}
            ...
            backIconButtonProps={{
              disabled: !internalPurchasesData?.pagination?.meta?.page?.isPrevious,
            }}
            nextIconButtonProps={{
              disabled: !internalPurchasesData?.pagination?.meta?.page?.isNext,
            }}
            dense={dense}
