...
const PurchaseForm = ({ purchaseData, setOpen }) => {
  ...
  // Suppliers Data
  const { data: rawSuppliersData } = useGetSuppliers();
  const suppliersData = useMemo(() => {
    return (
      rawSuppliersData?.data?.map(supplier => ({
        ...supplier,
        name: supplier.name[i18n.language] || supplier.name.en,
      })) || []
    );
  }, [rawSuppliersData, i18n.language]);
  ...
  const schema = yup.object({
    supplier_id: yup.string().required(() => i18n.t('validation.required')),
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      destination_type: purchaseData?.destination?.type || 'Warehouse',
  ...
  const onSubmit = handleSubmit(async formData => {
    ...
    try {
      ...
      const apiCall = purchaseData
        ...
        : PurchaseApiEndpoints.addPurchase({...data, type: 'external'});
      const response = await apiCall;
      navigate(`/inventory/purchase-details/${response?.data?.id}`);
      ToastSuccess(
  ...
  return (
    <Box component='form' noValidate onSubmit={onSubmit}>
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <CustomSelect
            label={t('inventory.purchase.labelSupplier')}
            placeholder={t('inventory.purchase.placeSupplier')}
            name='supplier_id'
            control={control}
            errors={errors}
